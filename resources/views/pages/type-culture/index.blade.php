        ...
        <div class="max-w-6xl mx-auto">
            <div class="mb-8 flex justify-between items-center">
                <div>
                    ...
                    <p class="text-emerald-600 font-medium ml-16">visualiser vos types de cultures </p>
                </div>
            </div>
            ...
            <div class="bg-white rounded-xl shadow-sm p-6 mb-8 border border-gray-100">
                <div class="flex flex-col sm:flex-row gap-5 justify-between items-start sm:items-center">
                    <div class="search-container w-full">
                        ...
                        <input type="text" id="searchInput" class="search-input" placeholder="Rechercher par libellé..."
                            autocomplete="off">
                    ...
                    <tbody id="tableBody">
                        @foreach ($typeCultures as $typeCulture)
                            <tr class="transition-all duration-300 ease-in-out">
                                <td>
                                    <span class="code-cell">{{ $typeCulture->code }}</span>
                                </td>
                                <td>{{ $typeCulture->libelle }}</td>
                                <td>
                                    <div class="flex gap-2">
                                        <form action="{{ route('type-culture.edit', ['type_culture' => Crypt::encrypt($typeCulture->id)]) }}" method="GET">
                                            @csrf
                                            <button type="submit"
                                        ...
                                        <form id="deleteForm-{{ $typeCulture->id }}"
                                            action="{{ route('type-culture.destroy', $typeCulture->id) }}" method="POST">
                                            @csrf
                                            @method('DELETE')
                                            <button data-id="{{ $typeCulture->id }}" type="submit"
            ...
            <div class="mt-5 text-sm text-gray-600 flex justify-between items-center">
                {{ $typeCultures->links('pagination::tailwind') }}
            </div>
        </div>
