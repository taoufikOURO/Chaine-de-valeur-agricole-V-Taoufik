                ...
                <div class="max-w-4xl mx-auto">
                    <div class="mb-8 flex justify-between items-center">
                        <div>
                            ...
                            <p class="text-emerald-600 font-medium ml-16">Récolter un semis</p>
                        </div>
                    </div>

                    <!-- Enhanced Form -->
                    <form action="{{ route('recolte.store') }}" method="POST"
                        ...
                        <div class="relative bg-gradient-to-b from-white to-gray-50 rounded-xl p-1 shadow-inner">
                            ...
                            <select type="text" required name="semis_id" required
                                ...
                                <option value="" disabled selected> Veuillez sélectionner le semis </option>
                                @foreach ($semis as $item)
                                    <option value="{{ $item->id }}">{{ $item->culture->nom }} | {{ $item->date_semis }}
                                    </option>
                                @endforeach
                            </select>
                        </div>
                        <div class="relative bg-gradient-to-b from-white to-gray-50 rounded-xl p-1 shadow-inner">
                            <div class="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                                <i class="fas fa-solid mr-2 fa-weight-hanging text-gray-400"></i>
                            </div>
                            <input type="number" required name="quantite_recolte" required
                                class="w-full pl-10 pr-4 py-3.5 rounded-xl border-0 ring-1 ring-gray-200 focus:ring-2 focus:border bg-transparent transition duration-200" placeholder="Quantité récoltée en tonne"/>
                        </div>
                        <button type="submit"
                            ...
                            <span class="relative">Récolter le semis</span>
                        </button>
                    </form>
                </div>
            </div>
