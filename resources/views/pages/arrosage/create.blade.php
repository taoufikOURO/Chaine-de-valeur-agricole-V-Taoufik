        ...
        <div class="flex min-h-screen">
            ...
            <div class="flex-1 p-8 bg-gradient-to-br from-green-50/50 to-emerald-100/50">
                <div class="max-w-4xl mx-auto">
                    ...
                    <!-- Enhanced Form -->
                    <form action="{{ route('arrosage.store') }}" method="POST"
                        ...
                        <div class="relative bg-gradient-to-b from-white to-gray-50 rounded-xl p-1 shadow-inner">
                            ...
                            <select name="semis_id" required
                                ...
                                <option value="" disabled selected> Veuillez sélectionnez votre semis
                                </option>
                                @foreach ($semis as $item)
                                    <option value="{{ $item->id }}">
                                        {{ $item->culture->nom }} | {{ $item->date_semis }}
                                    </option>
                                @endforeach
                            </select>
                            @error('semis_id')
                        ...
                        <button type="submit"
                            ...
                            <span class="relative">Arroser le semis</span>
                        </button>
                    </form>
                </div>
            </div>
